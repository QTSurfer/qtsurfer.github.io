...
dataset-backed prepare. Same `(prepareJobId, strategyId, storeSignals)` → same `jobId`
(idempotent).
...
| `equityCurve` | array of [`EquityPoint`](#visualizing-the-equity-curve) — present under the same condition as the yield metrics |
...
`results.equityCurve` is ready to plot directly — no Parquet download needed for this. Element 0
is an anchor at the backtest's `from` with the initial capital; every point after that is one
sample per emitted yield, each an [`EquityPoint`](../openapi.yaml): `timestamp` (epoch ms) and
