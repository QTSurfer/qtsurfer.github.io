...
| `GET` | `.../executeSweep/{requestId}/{sweepId}/runs/{runIx}/equityCurve` | A selected trial's curve — see the [shared equity-curve guide](equity_curves.md#sweeps-select-retain-and-fetch-curves) |
...
| `equityCurve` | [`EquityCurveRequest`](equity_curves.md#sweeps-select-retain-and-fetch-curves) | `{mode: "auto"}` | select retained trial curves and their default response transform |
...
in the [shared equity-curve guide](equity_curves.md#sweeps-select-retain-and-fetch-curves).
...
| `equityCurve` | present when this completed trial has a retained curve. It may contain a URL alone or a URL plus inline points; inspect `points`/`equities`, not object or URL presence, to tell. See [Equity curves](equity_curves.md#sweeps-select-retain-and-fetch-curves) |
...
The shared [equity-curve guide](equity_curves.md) covers sweep retention modes, leaderboard
